...
import { unitTestHelper } from '../support/unit-test-helper';
...
describe('Concat Scss', () => {
  beforeAll(() => {
    unitTestHelper.emptyOutputDirectory();
  });
  ...
  it('spec 4: should output css as well', (done) => {
    ...
    concatScss.concat(options).then((results) => {
      const output = results.output;
      unitTestHelper.getOutputFileContents('scss-spec-4.css', (contents) => {
        expect(contents.indexOf('color: #000;') > -1).toEqual(true);
        unitTestHelper.getOutputFileContents('scss-spec-4.scss', (contents) => {
